...
// The rest of this file was adapted from luvi's miniz bindings (see original copyright below)
...
static size_t lmz_file_read(void* pOpaque, mz_uint64 file_ofs, void* pBuf, size_t n) {
	lmz_file_t* zip = static_cast<lmz_file_t*>(pOpaque);
	const uv_buf_t buf = uv_buf_init(static_cast<char*>(pBuf), n);
...
static int lmz_writer_gc(lua_State* L) {
	lmz_file_t* zip = static_cast<lmz_file_t*>(luaL_checkudata(L, 1, "miniz_writer"));
...
static int lmz_reader_extract(lua_State* L) {
	...
	size_t out_len;
	char* out_buf = static_cast<char*>(mz_zip_reader_extract_to_heap(&(zip->archive), file_index, &out_len, flags));
...
static int lmz_writer_add_from_zip_reader(lua_State* L) {
	lmz_file_t* zip = static_cast<lmz_file_t*>(luaL_checkudata(L, 1, "miniz_writer"));
	lmz_file_t* source = static_cast<lmz_file_t*>(luaL_checkudata(L, 2, "miniz_reader"));
...
static int lmz_deflator_init(lua_State* L) {
	...
	lmz_stream_t* stream = static_cast<lmz_stream_t*>(lua_newuserdata(L, sizeof(*stream)));
...
static int lmz_deflator_gc(lua_State* L) {
	lmz_stream_t* stream = static_cast<lmz_stream_t*>(luaL_checkudata(L, 1, "miniz_deflator"));
	mz_deflateEnd(&(stream->stream));
	return 0;
}

static int lmz_inflator_gc(lua_State* L) {
	lmz_stream_t* stream = static_cast<lmz_stream_t*>(luaL_checkudata(L, 1, "miniz_inflator"));
...
static int lmz_inflator_deflator_impl(lua_State* L, lmz_stream_t* stream) {
	...
	miniz_stream->next_in = reinterpret_cast<const unsigned char*>(data);

	luaL_Buffer buf;
	luaL_buffinit(L, &buf);
	do {
		miniz_stream->avail_out = LUAL_BUFFERSIZE;
		miniz_stream->next_out = reinterpret_cast<unsigned char*>(luaL_prepbuffer(&buf));
		int status;
...
static int ltinfl(lua_State* L) {
	...
	char* out_buf = static_cast<char*>(tinfl_decompress_mem_to_heap(in_buf, in_len, &out_len, flags));
	lua_pushlstring(L, out_buf, out_len);
	free(out_buf);
	return 1;
}
...
static int ltdefl(lua_State* L) {
	...
	size_t out_len;
	int flags = luaL_optinteger(L, 2, 0);
	char* out_buf = static_cast<char*>(tdefl_compress_mem_to_heap(in_buf, in_len, &out_len, flags));
	lua_pushlstring(L, out_buf, out_len);
	free(out_buf);
	return 1;
}
