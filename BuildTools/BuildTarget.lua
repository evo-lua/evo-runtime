...
function BuildTarget:ProcessBinaryEmbeds()
	...
	for inputFilePath, outputFileName in pairs(self.autoGeneratedHeaders) do
		local outputFile = string.format("%s/%s", self.BUILD_DIR, outputFileName)
		ninjaFile:AddBuildEdge(outputFile, "embed " .. inputFilePath, {
			generated = "1",
		})
		table.insert(objectFiles, outputFile) -- Not needed? But deps aren't detected otherwise...
	end
end
