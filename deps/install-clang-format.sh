...
echo "The script will now attempt to install clang-format-$EVO_AUTOFORMAT_VERSION"
echo

# This should prompt the user, giving them time to inspect llvm.sh if so desired (not in CI runs)
sudo ./llvm.sh $EVO_AUTOFORMAT_VERSION
sudo apt install clang-format-$EVO_AUTOFORMAT_VERSION
echo
