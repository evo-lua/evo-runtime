...
FEATURE_FLAGS="-D BUILD_TESTING=OFF -D BUILD_SAMPLES=OFF -D BUILD_SHARED_LIBS=OFF -D BUILD_LUA_BINDINGS=ON -D BUILD_LUA_BINDINGS_FOR_LUAJIT=ON -D MATRIX_ROW_MAJOR=ON"
...
cmake -S $SRC_DIR -B $BUILD_DIR -G Ninja $FEATURE_FLAGS -DCMAKE_C_COMPILER=gcc $LIBRARY_INCLUDES -DCMAKE_BUILD_TYPE=Release -DCMAKE_OSX_ARCHITECTURES=$(uname -m)
cmake --build $BUILD_DIR --clean-first

cp $BUILD_DIR/libRmlCore.a $OUT_DIR
cp $BUILD_DIR/libRmlLua.a $OUT_DIR
