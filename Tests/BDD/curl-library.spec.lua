...
describe("curl", function()
	...
	describe("url_strerror", function()
		it("should be able to translate all available error codes", function()
			for index, errorName in ipairs(curl.errorNames) do
				local errorCode = ffi.C[errorName]
				local expectedErrorMessage = ffi.string(curl.bindings.curl_url_strerror(errorCode))
				assertEquals(curl.url_strerror(errorCode), expectedErrorMessage)
			end

			-- The mapped enum values should always be kept in sync
			local cEnumSize = tonumber(ffi.C.CURLUE_LAST) + 1 -- C enums start at 0
			assertEquals(#curl.errorNames, cEnumSize)
		end)
	end)
	...
	describe("url_set", function()
		...
		it("should be able to set escaped IPv6 zone IDs via the HOST part", function()
			...
			local href = "http://admin:topsecret@[fe80::20c:29ff:fe9c:409b%25eth0]:1234/hello/world?answer=42#bye"
			assert(url:set("host", "[fe80::20c:29ff:fe9c:409d]"))
			assert(url:get("host"))
			assertFailure(function()
				return url:get("zone")
			end, curl.url_strerror(ffi.C.CURLUE_NO_ZONEID))
			assertEquals(url:get("host"), "[fe80::20c:29ff:fe9c:409d]")
		end)
		...
		it("should be able to set escaped IPv6 zone IDs via the HOST part", function()
			...
			local href = "http://admin:topsecret@[fe80::20c:29ff:fe9c:409b%25eth0]:1234/hello/world?answer=42#bye"
			assert(url:set("host", "[fe80::20c:29ff:fe9c:4090%25eth3]"))
			assert(url:get("host"))
			assert(url:get("zone"))
			assertEquals(url:get("host"), "[fe80::20c:29ff:fe9c:4090%25eth3]")
		...
		-- it("should set the URL if no part name was provided", function()

		-- 	-- function curl.url_set(handle, what, part, how)
		-- 	-- 	what = what or "url" -- TODO swap order of what and part? allows shorthand, but diverges from curl API
		...
		-- it("should set the URL if an invalid part name was provided", function()
		-- 	local href  = "http://example.org"
		-- 	local url = curl.url()
		-- 	url:set(href, "invalid123")
		-- 	assert(url:get("url"))
		-- 	assertEquals(url:get("url"), href)
		-- end)

		-- TBD other flags? TODO combine them... curl.features(names, "on|off|toggle|status|list")
		...
		-- it("should be able to , function() -- TODO decode also (get)
		-- 	-- TBD does not work for IPv6
		-- 	local url = curl.url()
		-- 	url:set("url", href, ffi.C.CURLU_DEFAULT_FEATURES)
		-- 	error("NYI")
		-- 	local encodedParts =
		-- 		"https://sb%C3%B8mw%C3%B8llen:letmein@[fe80::1%25eth0]:8080/path/to/success?a=1&b=2#section1"
		-- end)
	end)
